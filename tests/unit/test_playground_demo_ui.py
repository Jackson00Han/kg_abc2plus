...
assert.equal($('construct-button').textContent, '上传并构建');
assert.match($('construction-submit-note').textContent, /先复核，再发布/);
$('document-extraction-mode').value='SOURCE_ONLY';updateConstructionMode();
assert.equal($('construct-button').textContent, '上传并保存来源');
assert.match($('construction-submit-note').textContent, /不生成候选知识/);
