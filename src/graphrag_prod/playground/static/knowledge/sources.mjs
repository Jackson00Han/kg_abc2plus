...
export function mountSources({api,epoch,browser,maintain,feedback=()=>()=>{}}){
  ...
  async function load(button=$('kb-source-refresh')){
    const finish=feedback(button);
    const id=++request,identity=epoch();$('kb-source-list').replaceChildren();$('kb-source-summary').textContent='';
    try{const result=await api('/v1/knowledge/sources:query',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({limit:30,...(after?{after}:{})})});if(id!==request || identity!==epoch())return;
      ...
      $('kb-source-list').querySelectorAll('[data-open]').forEach(b=>b.onclick=()=>{const r=rows[Number(b.dataset.open)];return openDocument(r.document_id,r.version_id,0,b);});
      $('kb-source-list').querySelectorAll('[data-knowledge]').forEach(b=>b.onclick=()=>knowledge(rows[Number(b.dataset.knowledge)]));
    }catch(error){if(id===request && identity===epoch()){$('kb-source-summary').textContent=`资料读取失败：${error.message}`;$('kb-source-next').disabled=true;}}finally{finish();$('kb-source-prev').disabled=!history.length;$('kb-source-next').disabled=!next;}
  }
  ...
  async function openDocument(document_id,version_id,ordinal=0,button=$('kb-source-refresh')){
    const finish=feedback(button);
    const id=++detailRequest,identity=epoch();current=null;$('kb-source-detail').textContent='';
    try{const r=await api('/v1/knowledge/sources:read',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({document_id,version_id,ordinal})});if(id!==detailRequest || identity!==epoch())return;current=r;
      ...
      $('kb-source-detail').querySelector('[data-before]').onclick=event=>openDocument(document_id,version_id,ordinal-1,event.currentTarget);$('kb-source-detail').querySelector('[data-after]').onclick=event=>openDocument(document_id,version_id,ordinal+1,event.currentTarget);$('kb-source-detail').querySelector('[data-facts]').onclick=()=>knowledge(r);$('kb-source-detail').querySelector('[data-maintain]').onclick=()=>maintain(r);
    }catch(error){if(id===detailRequest && identity===epoch())$('kb-source-detail').textContent=error.status===409?'来源版本、完整性或访问权限已变化，请刷新资料列表。':`原文读取失败：${error.message}`;}finally{finish();}
  }
  $('kb-source-refresh').onclick=()=>{reset();load();};$('kb-source-next').onclick=()=>{history.push(after);after=next;load($('kb-source-next'));};$('kb-source-prev').onclick=()=>{after=history.pop()??null;load($('kb-source-prev'));};
