...
export function mountMaintenance({api,epoch,browser,navigate,correct}){
  ...
  function openObject(issue){navigate('browse');browser.tab('entities');const d=browser.getDirectory();const n=d?.items.find(n=>n.entity_id===issue.object_id || n.mention_revision_ids.includes(issue.object_id) || [...n.properties,...n.relations].some(f=>f.revision_id===issue.object_id || f.record_id===issue.object_id));if(n)browser.select(n.entity_id);else browser.load();}
