...
export function mountBrowser({api,epoch,maintain}) {
  ...
  function select(entityId){
    ...
    const property=n.properties.map(f=>`<div class="kb-fact"><strong>${e(label(f.predicate))}</strong>：${e(valueLabel(f))}<div class="kb-muted">${e(contextLabel(f))} ${f.authority_level==='AUTHORITATIVE'?'权威来源':'业务来源'}</div><button class="button" data-evidence="${e(f.revision_id)}">查看依据</button></div>`).join('');
    const relations=n.relations.map(f=>`<div class="kb-fact">${f.direction==='incoming'?'来自':'指向'} <button class="button" data-entity="${e(f.other.entity_id)}">${e(f.other.label)}</button> · ${e(label(f.predicate))}<br><button class="button" data-evidence="${e(f.revision_id)}">查看关系依据</button></div>`).join('');
