...
export function mountBrowser({api,epoch,maintain,feedback=()=>()=>{},onExplore=null,onSource=null}) {
  ...
  async function load(version_filter=scope,title=scopeTitle,button=$('kb-refresh')){
    ...
    $('kb-list').replaceChildren();$('kb-dossier').textContent='';$('kb-summary').textContent='';
    try{
    ...
    }catch(error){if(current(id,identity)){$('kb-summary').textContent=error.status===403?'当前身份没有知识浏览权限。':`知识读取失败：${error.message}。可按来源缩小范围后重试。`;$('kb-dossier').textContent='未显示不完整或过期的知识。';}}finally{finish();}
  }
  ...
  $('kb-refresh').onclick=()=>load();$('kb-clear-scope').onclick=()=>load({},'',$('kb-clear-scope'));$('kb-search').oninput=()=>{page=0;renderList();};$('kb-type').onchange=()=>{page=0;renderList();};
