...
export function comparisonMarkup(result){
  ...
  const sourceRow=item=>`<li><strong>${e(item.title)}</strong><details><summary>文档版本</summary><p>${e(item.document_id)} · ${e(item.version_id)}</p></details></li>`;
  const sources=scope?`<h4>正式检索的文档范围</h4><p>加入 ${scope.added.length} 个文档版本、退出 ${scope.removed.length} 个文档版本；保持 ${scope.unchanged_count} 个。</p><details ${scope.added.length?'open':''}><summary>加入检索</summary><ul>${scope.added.map(sourceRow).join('')||'<li>无</li>'}</ul></details><details ${scope.removed.length?'open':''}><summary>退出检索</summary><ul>${scope.removed.map(sourceRow).join('')||'<li>无</li>'}</ul></details>`:'';
  return `${sources}<h4>实体、关系与属性</h4><p>切换后新增 ${result.added.length} 条、移除 ${result.removed.length} 条、变更 ${result.changed.length} 条；保持 ${result.unchanged_count} 条。</p><h4>新增</h4><ul>${result.added.map(row).join('')||'<li>无</li>'}</ul><h4>移除</h4><ul>${result.removed.map(row).join('')||'<li>无</li>'}</ul><h4>变更</h4><ul>${result.changed.map(c=>`<li><p>当前：${e(factTitle(c.before))}</p><p>目标：${e(factTitle(c.after))}</p><p>来源：${e(c.before.document_title)} → ${e(c.after.document_title)}</p><small>内容、来源依据或记录版本发生变化。</small></li>`).join('')||'<li>无</li>'}</ul>`;
}
export function mountActions({api,epoch,browser,feedback=()=>()=>{},navigate,correctRecord,rollback,toast,maintainSource}){
  ...
  function open(title,content,show=true){reset();dialog.innerHTML=`<h3>${e(title)}</h3>${content}<p data-status role="status"></p><div class="kb-actions"><button class="button" data-close>关闭</button></div>`;dialog.querySelector('[data-close]').onclick=reset;if(show)dialog.showModal();return {id:serial,identity:epoch()};}
  ...
  async function correct(recordId,revisionId=null,button=null){
    const finish=feedback(button);
    const pin={id:serial,identity:epoch()};const status=dialog.querySelector('[data-status]');
    try{if(status)status.textContent='';await correctRecord(recordId,revisionId);if(valid(pin))reset();}
    catch(error){if(valid(pin) && status)status.textContent=errorText(error);else if(pin.identity===epoch())toast(errorText(error));}finally{finish();}
  }
  async function maintain(item){
    ...
    dialog.querySelectorAll('[data-proof]').forEach(b=>b.onclick=()=>browser.evidence([choices[Number(b.dataset.proof)].revision_id],token,0,b));
    dialog.querySelectorAll('[data-correct]').forEach(b=>b.onclick=async()=>{
      const finish=feedback(b);b.disabled=true;const choice=choices[Number(b.dataset.correct)];
      try{
      ...
      }catch(error){if(valid(pin)){if(!dialog.open)dialog.showModal();dialog.querySelector('[data-status]').textContent=errorText(error);}}finally{finish();if(valid(pin))b.disabled=false;}
    });
  }
  function inventory(inventory,container,summary,selected,onSelect,onHistory){
    ...
    container.querySelectorAll('[data-correct-record]').forEach(b=>b.onclick=()=>{open('修正已发布知识',`<p>${e(factTitle(inventory.items[Number(b.dataset.correctRecord)]))}</p><p>开始后生成待修订记录，当前检查可能提示版本不一致。完成审核并重新发布后恢复一致。</p><button class="button primary" data-begin>开始修正</button>`);dialog.querySelector('[data-begin]').onclick=event=>correct(inventory.items[Number(b.dataset.correctRecord)].record_id,inventory.items[Number(b.dataset.correctRecord)].revision_id,event.currentTarget);});
    container.querySelectorAll('[data-history]').forEach(b=>b.onclick=()=>revisionHistory(inventory.items[Number(b.dataset.history)],b));
  }
  async function revisionHistory(item,button){
    const finish=feedback(button);
    const pin=open('知识修订历史',`<p>${e(factTitle(item))}</p><div data-history-content></div>`,false);
    ...
    }catch(error){if(valid(pin)){if(!dialog.open)dialog.showModal();dialog.querySelector('[data-status]').textContent=errorText(error);}}finally{finish();}
  }
  ...
  async function compare(target,active,button){
    const finish=feedback(button);
    const pin=open(`将第 ${target.generation} 版设为当前版本`,`<div data-comparison></div>`,false);
    try{const result=await api('/v1/knowledge/publications:compare',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({target_publication_id:target.publication_id,expected_active_publication_id:active.publication_id})});if(!valid(pin))return;
      if(!dialog.open)dialog.showModal();
      dialog.querySelector('[data-comparison]').innerHTML=`${comparisonMarkup(result)}<p>确认后，正式检索、问答和图谱统一使用第 ${e(target.generation)} 版的文档内容及实体、关系、属性。退出当前版本的内容保留在历史中，可通过版本切换重新启用；原文与审核记录不会删除。</p><p>已撤回的来源不会自动恢复。来源、权限或知识模型不满足条件时，系统会阻止切换。</p><button class="button primary" data-confirm>确认将第 ${e(target.generation)} 版设为当前版本</button>`;
      dialog.querySelector('[data-confirm]').onclick=async()=>{if(!valid(pin))return;const b=dialog.querySelector('[data-confirm]');if(b.disabled)return;const done=feedback(b);b.disabled=true;try{await rollback(target.publication_id,active.publication_id);if(valid(pin))reset();}catch(error){if(valid(pin)){dialog.querySelector('[data-status]').textContent=errorText(error);b.textContent='切换未完成，请关闭后重新比较';}}finally{done();if(valid(pin))b.disabled=true;}};
    }catch(error){if(valid(pin)){if(!dialog.open)dialog.showModal();dialog.querySelector('[data-status]').textContent=errorText(error);}}finally{finish();}
  }
