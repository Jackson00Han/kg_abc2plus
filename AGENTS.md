...
| 8. Automated evaluation and regression gates | Complete | `docs/automated_evaluation.md`; `docs/validation/stage-8.md`; current baseline 1.4.0 covers 649 unit + 15 HTTP E2E + 33 security + 2 regression + 125 disposable-Neo4j tests plus the locked industrial extraction-quality gate; published completion and later baseline maintenance are recorded separately; the corrected live-provider workflow is recorded in `docs/validation/extraction-timeout-correction.md`, with browser/production limits retained |
