...
本轮仅记录问题，不改写 `new_files/`。这些事项是该输入包的接入准备，不是所有领域共享的硬编码约束。

- 六份文件现已齐备。诊断条目实测为 69 实体、124 关系，类型和引用结构闭合；XML 与测点映射的 64 个测点对应一致。这些是有限只读检查，尚未通过完整导入、语义与运行验收。
- 本体包含包清单、固定数量和具体规则/实例 ID 对账。应分离为本体、包清单、规则配置与样本验收基线；本体可独立管理，知识条目按本体校验。
- 本体与 XML 映射的派生边证据字段/关系 ID 约定存在冲突。接入时统一稳定应用 ID 与来源证据，禁止采用 Neo4j 内部 ID 作为持久业务标识。
- 规则版本节点的来源当前指向拓扑；相关规则关系还主要引用外部知识记录。应分别绑定实际规则文件的不可变版本及精确条款，不能把“标识引用闭合”当作“来源语义正确”。
- 原始业务知识记录尚未随包提供。当前可以将用户提供的整理文件登记为来源，但不能宣称已核验其所引用原文或外部行业权威；补资料须遵守授权范围。
- `rule_v2.md` 含 staging/mock 条款与参数；`point_mapping.yaml` 为未激活的测试绑定。测试与生产适用范围必须过滤到检索与执行，不能因问答引用而提升为生产判据。
- `EXPECTED_OBSERVABLE` 与 `EXPOSES_OBSERVABLE` 适合验证“应有/已有”及缺口；`MAY_EXHIBIT`、`MAY_AFFECT` 明确禁止无条件实例扩展，必须保持其模板语义。

输入依据：[本体](../new_files/ontology.yaml)、[知识条目](../new_files/instances.diagnostic.yaml)、[拓扑](../new_files/topology.xml)、[导入映射](../new_files/mapping.topology_xml.yaml)、[规则](../new_files/rule_v2.md)、[取数绑定](../new_files/point_mapping.yaml)。

## 9. 本项目实施顺序与验收边界

| 阶段 | 本项目交付 | 验收重点 | 外部依赖 |
| --- | --- | --- | --- |
| K1 输入与合同 | 本体独立管理、知识包清单、来源分工、语义/格式校验与接口映射设计 | 缺失/冲突可定位，源文件不丢语义，测试知识与正式知识隔离 | 领域专家确认定义、平台评审公共合同 |
| K2 确定性构建 | XML/YAML 映射、稳定身份、精确证据、候选审核和新知识库发布 | 幂等、无跨项目误合并、权限、更新/撤回一致性 | 用户授权的新包及经确认的源快照 |
| K3 语义与可视化 | 分类查询、有限路径/范围派生、实际/应有/可能分开、知识缺口展示 | 原因不复制，条件不足不推导，规则变更/撤回使结果失效 | 受支持的明确派生定义；UI 改动按 AGENTS 做真实浏览器验收 |
| K4 检索与 Agent 接口 | 专业条款检索、证据封装、跨调用版本固定、DeerFlow 接口适配或契约夹具 | 引用准确、冲突保留、越权拒绝、预算截断、工作流一致性 | DeerFlow 部署与委托身份合同；未提供时只做契约级验证 |
| K5 首个场景联调 | 向领域分析提供对象、范围、规则和证据，验证返回结果可追溯 | 本库验收知识贡献；平台另验收数据、规则执行、最终答案和任务闭环 | 外部诊断执行器、有效参数、数据适配、平台 UI |

公共能力验收至少包括两种本体配置下的同一导入/检索接口、同名异物、跨项目/租户拒绝、来源撤回、未知类型/操作、缺参数/缺测点、部分返回和超时。当前先使用已授权材料及确定性夹具；未来真空断路器等真实语料进入后再做对应领域验收，不能把母线槽样本结果宣称为跨领域已验证。

知识库不为完成 K1–K4 提前建设时序库、采集网关、规则运行引擎、Agent 编排框架或完整诊断 UI。新领域允许复用本体/映射配置和已有操作；确有新算法时，由对应责任层增加受测试的实现，知识库只补相应语义与证据接口。

## 10. 文档与图的维护

架构图使用 Graphviz，编辑源文件后执行：

```sh
dot -Tsvg docs/architecture/platform-overview.dot -o docs/architecture/platform-overview.svg
dot -Tpng -Gdpi=144 docs/architecture/platform-overview.dot -o docs/architecture/platform-overview.png
git diff --check
```

修改架构边界时同步核对 `AGENTS.md`。检查相对链接并实际查看渲染图；文档检查不代表应用联调。新文件包维持原始字节，本次架构工作不执行导入、发布、数据库重置、服务切换或外部工具部署。

2026-09-13 文档检查：相对链接检查、API 路由名称核对、Graphviz SVG/PNG 渲染与图片目视检查、SVG XML 解析、新增文本凭据模式扫描、已跟踪及新增文本的差异格式检查均通过。本次为纯文档修改，未运行应用测试、Neo4j 集成测试或 8002 业务浏览器验收。
